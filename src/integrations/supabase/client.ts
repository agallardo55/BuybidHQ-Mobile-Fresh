...
logger.debug('🔍 Supabase client: Module loaded, imports completed');

const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL;
const SUPABASE_PUBLISHABLE_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY;

logger.debug('🔍 Supabase client: Environment variables checked', { 
  hasUrl: !!SUPABASE_URL, 
...
logger.debug('🔍 Supabase client: Creating client instance...');
...
if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) {
  ...
  logger.warn('⚠️ Supabase client: Dummy client created - Supabase features will not work');
} else {
  try {
    ...
    logger.debug('✅ Supabase client: Client instance created');
  } catch (error) {
    logger.error('❌ Supabase client: Failed to create client', error);
    // Don't throw - create a dummy client to prevent blocking
    logger.warn('⚠️ Supabase client: Creating dummy client after error');
    supabase = createClient<Database>('https://dummy.supabase.co', 'dummy-key', {
      auth: { persistSession: false, autoRefreshToken: false }
    }) as any;
  }
...
logger.debug('✅ Supabase client: Module initialization complete');
