-- Add body_style to get_public_bid_request_details RPC function
...
CREATE OR REPLACE FUNCTION public.get_public_bid_request_details(p_token text)
 RETURNS TABLE(
   ...
   vehicle_transmission text, vehicle_drivetrain text, vehicle_body_style text,
   ...
   history text, history_service text, book_values_condition text
 )
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Return bid request details including body_style, book values, condition data, and history
  RETURN QUERY
  SELECT
    br.id as request_id,
    br.created_at,
    ...
    COALESCE(v.body_style, 'N/A') as vehicle_body_style,
    ...
    COALESCE(r.history, 'noAccidents') as history,
    COALESCE(v.history_service, 'Unknown') as history_service,
    COALESCE(bv.condition, 'excellent') as book_values_condition
...
COMMENT ON FUNCTION public.get_public_bid_request_details(text) IS
'Returns public bid request details for buyers via secure token. Includes vehicle info, body style, condition data, history, and book values.';
