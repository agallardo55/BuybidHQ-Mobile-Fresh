-- Update get_public_bid_request_details to prioritize individual_dealers over dealerships
CREATE OR REPLACE FUNCTION public.get_public_bid_request_details(p_token text)
 RETURNS TABLE(request_id uuid, created_at timestamp with time zone, status text, vehicle_year text, vehicle_make text, vehicle_model text, vehicle_trim text, vehicle_vin text, vehicle_mileage text, vehicle_engine text, vehicle_transmission text, vehicle_drivetrain text, vehicle_exterior_color text, vehicle_interior_color text, vehicle_accessories text, buyer_name text, buyer_dealership text, buyer_mobile text, is_used boolean, submitted_offer_amount numeric, submitted_at timestamp with time zone, vehicle_images json, kbb_wholesale numeric, kbb_retail numeric, jd_power_wholesale numeric, jd_power_retail numeric, mmr_wholesale numeric, mmr_retail numeric, auction_wholesale numeric, auction_retail numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Return sanitized bid request details for this specific token including colors, accessories, and book values
  -- Now prioritizes individual_dealers.business_name over dealerships.dealer_name
  RETURN QUERY
  SELECT 
    br.id as request_id,
    br.created_at,
