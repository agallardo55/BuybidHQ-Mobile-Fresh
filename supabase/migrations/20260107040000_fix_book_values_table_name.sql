-- Fix book_values table name in get_public_bid_request_details function
-- The function was using "bookValues" (case-sensitive) instead of book_values
...
BEGIN
  ...
  -- Return bid request details including book values, condition data, and history
  RETURN QUERY
  SELECT
    br.id as request_id,
    br.created_at,
    ...
    v.history_service as history_service,
  ...
  LEFT JOIN book_values bv ON bv.vehicle_id = v.id  -- FIXED: was "bookValues"
...
'Returns public bid request details for buyers via secure token. Includes vehicle info, condition data, history, and book values.';
